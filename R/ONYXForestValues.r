ONYXForestValues <- function(estim, upper, lower, stderr=NULL, isLogged=TRUE, xaxmax, YLocs, Boldness, mainfont, separator=", ", Show, ShowCI, spacing=0, digits=NULL, digits_SE=NULL, ValueLabelsEffectSE=FALSE) {
	...
	for (i in 1:nlabs) {
		...
		# NW: Don't move position of text if estimate is negative
		...
		# NW: Just plot Estimate and CI at this position (i.e. do not leave additional space for minus signs)
		ci_part <- paste(ValueLabels[[1]][i], " (", ValueLabels[[2]][i], separator, ValueLabels[[3]][i], ")", sep="")
		text(x=xcoord, y=YLocs[i], ci_part, font=Boldness[i], adj=c(0, 0.5), cex=Font)
